...
namespace Kaede2.Scenario.Framework.Commands
{
    public class AutoLoad : Command
    {
            ...
            foreach (var statement in Module.Statements)
            {
                ...
                switch (statementArgs[0])
                {
                    ...
                    case "actor_setup":
                    {
                        ...
                        allLoadData.Add(new(ScenarioModule.Resource.Type.Actor, resourceName));
                        break;
                    }
                    case "font":
                    {
                    ...
                    case "sprite":
                    {
                        string argWithResource = statementArgs[1];
                        string resourceName = Module.ResolveAlias(argWithResource.Split(':')[0]);
                        allLoadData.Add(new(ScenarioModule.Resource.Type.Sprite, resourceName));
                        break;
                    }
                    case "still":
                    {
                        ...
                        allLoadData.Add(new(ScenarioModule.Resource.Type.Still, still));
                        break;
                    }
                    case "背景":
                    case "bg":
                    case "replace":
                    {
                        ...
                        allLoadData.Add(new(ScenarioModule.Resource.Type.Background, bg));
                        break;
                    }
                    case "se":
                    // case "se_load":
                    case "se_loop":
                    {
                        ...
                        allLoadData.Add(new(ScenarioModule.Resource.Type.SE, assetName));
                        break;
                    }
                    case "bgm":
                    // case "bgm_load":
                    {
                        string assetName = Module.ResolveAlias(statementArgs[1]);
                        allLoadData.Add(new(ScenarioModule.Resource.Type.BGM, assetName));
                        break;
                    }
                    case "mes":
                    case "mes_auto":
                    {
                        allLoadData.Add(new(ScenarioModule.Resource.Type.Voice, statementArgs[2]));
                        break;
                    }
                    // case "voice_load":
                    // {
                    ...
                    case "transform_prefab":
                    {
                        ...
                        allLoadData.Add(new(ScenarioModule.Resource.Type.TransformPrefab, id));
                        break;
                    }
                }
            }
        ...
        {
            public readonly ScenarioModule.Resource.Type Type;
            public readonly string Name;

            public LoadData(ScenarioModule.Resource.Type type, string name)
            {
                Type = type;
                Name = name;
            }
