...
half4 GetBlurInDir(v2f IN, half4 pixel, sampler2D tex, float4 size, half dirx, half diry)
{
    ...
    float sigma = radius / 3; // A common choice to cover 99% of the Gaussian bell curve

    // Include the center pixel
    float weight = exp(-0.0); // Center pixel, distance is 0, so weight is 1
    sum += weight * GrabPixel(tex, IN.uvgrab);
    totalWeight += weight;

    // Loop over pixels in the specified direction
    for (half range = ITER_STEP; range <= radius; range += ITER_STEP)
    {
        weight = exp(-(range * range) / (2 * sigma * sigma));
        sum += weight * GrabPixelXY(tex, IN.uvgrab, size, range*dirx, range*diry);
        sum += weight * GrabPixelXY(tex, IN.uvgrab, size, -range*dirx, -range*diry);
